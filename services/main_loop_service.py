...
class MainLoopService(LoggerMixin):
    ...
    def choose_party_member(self) -> int:
        self.console_service.print_ascii("Player: ", include_carriage_return = False)

        selected_member_index = 0
        self.display_service.highlight_party_member(selected_member_index)

        party_count = len(self.party_agent.get_party_members())
        ...
        while self._is_running:
            ...
            if event.key == pygame.K_UP:
                selected_member_index = (selected_member_index - 1) % party_count

            elif event.key == pygame.K_DOWN:
                selected_member_index = (selected_member_index + 1) % party_count

            elif event.key == pygame.K_RETURN:
                # Pressing enter will return the chosen party member index.
                self.display_service.highlight_party_member(None)
                chosen_party_member = self.party_agent.get_party_members()[selected_member_index]
                self.console_service.print_ascii(chosen_party_member.name + " !")
            ...
            # Update the highlighted party member info.
            self.display_service.highlight_party_member(selected_member_index)
