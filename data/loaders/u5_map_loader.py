...
class U5MapLoader(LoggerMixin):
    ...
    def register_maps(self):
        for trigger_index in range(self.get_number_locations()):
            ...
            self.global_registry.maps.register(u5map.location_metadata.location_index, u5map)
            self.log(f"Loaded map {u5map.location_metadata.name} containing {len(u5map.levels)} levels.")

        world = self.build_world_map()
        self.log(f"Loaded map {world.location_metadata.name} containing {len(world.levels)} levels.")
        self.global_registry.maps.register(world.location_metadata.location_index, world)
